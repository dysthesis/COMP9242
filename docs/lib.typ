...
#let script-size = 6.97224pt
#let footnote-size = 6.50012pt
#let small-size = 5.24994pt
#let normal-size = 6pt
#let large-size = 8.74988pt
...
#let ams-article(
  ...
  set page(
    ...
    header: context {
      ...
      grid(
        ...
        if calc.even(i) [#i], upper(if calc.odd(i) { title } else { author-string }), if calc.odd(i) { [#i] },
      )
    },
  ...
  set list(indent: 12pt, body-indent: 5pt)
  set enum(indent: 12pt, body-indent: 5pt)
...
#let theorem(body, numbered: true) = figure(body, kind: "theorem", supplement: [Theorem], numbering: if numbered {
  n => counter(heading).display() + [#n]
})
