...
seL4_CPtr client_get_vspace(client_t *client) {
    printf("[client_get_vspace] client=%p user_client=%p id=%d/%d gen=%u/%u "
           "vspace=%#lx\n",
           (void *)client, (void *)user_process.client,
           client ? client->id : -1,
    ...
    if (client && user_process.client &&
        client->id == user_process.client->id &&
...
NORETURN void syscall_loop(seL4_CPtr ep) {
    ...
    while (1) {
        ...
        if (badge & IRQ_EP_BADGE) {
            ...
            if (!caller) {
                ZF_LOGE("Unknown/stale caller badge=0x%lx",
                        (unsigned long)badge);
                have_reply = false;
                continue;
            }
...
static void cleanup_stack_frames(void) {
    ...
    if (use_vm_reset) {
        ...
        while (user_process.stack_frame_count > 0) {
            ...
            seL4_CPtr slot =
                user_process.stack_slots[user_process.stack_frame_count];
...
static int map_process_stack_page(uintptr_t vaddr) {
    if (user_process.stack_frame_count >=
        ARRAY_SIZE(user_process.stack_frames)) {
    ...
    struct vm_handle *vm_handle =
        (user_process.client && user_process.client->vm_state)
            ...
            : NULL;
    if (vm_handle != NULL) {
        int vm_err = vm_map_owned_frame(vm_handle, vaddr, frame, slot, true,
                                        true, false, true, true);
        if (vm_err < 0) {
            if (vm_err == -EEXIST) {
                ZF_LOGE("Stack frame already mapped at %p", (void *)vaddr);
            } else {
                ZF_LOGE("VM stack map failed for %p errno=%d", (void *)vaddr,
                        -vm_err);
            }
...
bool start_first_process(char *app_name, seL4_CPtr ep) {
    ...
    err =
        cspace_copy(&cspace, user_process.ipc_buffer, frame_table_cspace(),
    ...
    err = cspace_mint(&user_process.cspace, user_ep, &cspace, ep,
...
static void sos_ipc_init(seL4_CPtr *ipc_ep, seL4_CPtr *ntfn) {
    ...
    ut_t *ut =
        alloc_retype(ntfn, seL4_NotificationObject, seL4_NotificationBits);
