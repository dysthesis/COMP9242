...
static bool nfs_mount_complete = false;
static int dhcp_status = DHCP_STATUS_WAIT;
static char nfs_dir_buf[PATH_MAX];
static uint8_t ip_octet;

static void nfs_mount_cb(int status, struct nfs_context *nfs, void *data,
                         void *private_data);

static int pico_eth_send(UNUSED struct pico_device *dev, void *input_buf,
                         int len) {
  if (ethif_send(input_buf, len) != ETHIF_NOERROR) {
    /* If we get an error, just report that we didn't send anything */
    return 0;
  }
  /* Currently assuming that sending always succeeds unless we get an error
   * code. Given how the u-boot driver is structured, this seems to be a safe
   * assumption. */
  return len;
}
...
static int pico_eth_poll(UNUSED struct pico_device *dev, int loop_score) {
  while (loop_score > 0) {
    int len;
    int retval =
        ethif_recv(&len); /* This will internally call 'raw_recv_callback'
...
void raw_recv_callback(uint8_t *in_packet, int len) {
  /* Note that in_packet *must* be copied somewhere in this function, as the
   * memory will be re-used by the ethernet driver after this function returns.
   */
...
void nfslib_poll() {
  ...
  struct pollfd pfd = {.fd = nfs_get_fd(nfs), .events = nfs_which_events(nfs)};
...
static int network_irq(UNUSED void *data, UNUSED seL4_Word irq,
...
static int network_tick(UNUSED void *data, UNUSED seL4_Word irq,
                        seL4_IRQHandler irq_handler) {
...
static void init_irq(int irq_number, int edge_triggered,
                     sos_irq_callback_t callback) {
  seL4_IRQHandler irq_handler = 0;
  int init_irq_err = sos_register_irq_handler(irq_number, edge_triggered,
                                              callback, NULL, &irq_handler);
...
void network_init(cspace_t *cspace, void *timer_vaddr, seL4_CPtr irq_ntfn) {
  ...
  uint8_t mac_addr[6];
  error =
      ethif_init(eth_base_vaddr, mac_addr, &ethif_dma_ops, &raw_recv_callback);
  ...
  /* Configure a watchdog IRQ for 1 millisecond from now. Whenever the watchdog
   * is reset using watchdog_reset(), we will get another IRQ 1ms later */
  ...
  do {
    ...
    if (dhcp_status == DHCP_STATUS_ERR) {
      ...
      error =
          pico_dhcp_initiate_negotiation(&pico_dev, dhcp_callback, &dhcp_xid);
      ZF_LOGF_IF(error != 0, "Failed to initialise DHCP negotiation");
    }
...
void nfs_mount_cb(int status, UNUSED struct nfs_context *nfs, void *data,
  ...
  nfs_mount_complete = true;
}

struct nfs_context *get_nfs_context(void) { return nfs; }

bool nfs_is_mounted(void) { return nfs_mount_complete; }
