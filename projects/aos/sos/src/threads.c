...
sos_thread_t *thread_create(thread_main_f function, void *arg, seL4_Word badge,
  ...
  new_thread->ipc_buffer_ut = alloc_retype(
      &new_thread->ipc_buffer, seL4_ARM_SmallPageObject, seL4_PageBits);
  ...
  seL4_Word err = cspace_mint(&cspace, new_thread->user_ep, &cspace, ipc_ep,
  ...
  new_thread->tcb_ut =
      alloc_retype(&new_thread->tcb, seL4_TCBObject, seL4_TCBBits);
  ...
  /* Configure the TCB */
  err = seL4_TCB_Configure(new_thread->tcb, cspace.root_cnode, seL4_NilData,
  ...
  new_thread->sched_context_ut =
      alloc_retype(&new_thread->sched_context, seL4_SchedContextObject,
  ...
  if (debugger_add) {
    ...
    err = cspace_mint(&cspace, new_thread->fault_ep, &cspace, fault_ep,
  ...
  err = map_frame(&cspace, new_thread->ipc_buffer, seL4_CapInitThreadVSpace,
