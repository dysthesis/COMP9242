...
/*
 * This function will be called from the network stack on any networking event,
 * such as receiving data. If you have a registered handler to receive incoming
 * data, it gets called from here.
 */
static void network_console_recv_handler(uint16_t ev, struct pico_socket *s) {
    if (ev == PICO_SOCK_EV_RD && network_console.handler) {
        int read = 0;
        do {
            read = pico_socket_recvfrom(network_console.pico_socket, buf,
                                        MAX_PAYLOAD_SIZE, &network_console.peer,
                                        &network_console.port);
...
struct network_console *network_console_init(void) {
    ...
    network_console.pico_socket = pico_socket_open(
        PICO_PROTO_IPV4, PICO_PROTO_UDP, &network_console_recv_handler);
    ...
    int err = pico_socket_bind(network_console.pico_socket,
                               &network_console.inaddr_any, &port_be);
    if (err) {
        return NULL;
    }

    err = pico_socket_connect(network_console.pico_socket,
                              &network_console.peer, network_console.port);
    if (err < 0) {
...
int network_console_send(struct network_console *network_console,
    ...
    while (total_sent < len) {
        int sent =
            pico_socket_sendto(network_console->pico_socket, data, len,
                               &network_console->peer, network_console->port);
        if (sent == -1) {
