...
static void handle_read_regs(char *output) {
  ...
  int error = seL4_TCB_ReadRegisters(
      target_thread->tcb, true, 0, sizeof(seL4_UserContext) / sizeof(seL4_Word),
      &context);
...
static void handle_write_regs(char *ptr, char *output) {
  ...
  int error = seL4_TCB_WriteRegisters(
      target_thread->tcb, true, 0, sizeof(seL4_UserContext) / sizeof(seL4_Word),
      &context);
...
static void handle_query(char *ptr, char *output) {
  if (strncmp(ptr, "qSupported", 10) == 0) {
    ...
    snprintf(output, BUFSIZE,
             "qSupported:PacketSize=%lx;QThreadEvents+;swbreak+;hwbreak+;"
             "vContSupported+;fork-events+;exec-events+;multiprocess+;",
             BUFSIZE);
...
/* Expected string is of the form [Mm][a-fA-F0-9]{sizeof(seL4_Word) *
 * 2},[a-fA-F0-9] +*/
...
static bool parse_breakpoint_format(char *ptr, seL4_Word *addr,
                                    seL4_Word *kind) {
  ...
  *kind = 0;
  ...
  /* This is generally to do with the size of the breakpoint that is set. This
     will be 4 for breakpoints and any of the following for watchpoints */
...
static void handle_configure_debug_events(char *ptr, char *output) {
  ...
  if (strncmp(ptr, "Z0", 2) == 0) {
    ...
    if (ptr[0] == 'Z') {
      success = set_hardware_watchpoint(target_thread->inferior, addr,
                                        watchpoint_type, size);
    } else {
      success = unset_hardware_watchpoint(target_thread->inferior, addr,
                                          watchpoint_type, size);
    }
  }
...
static gdb_thread_t *lookup_thread_from_id(gdb_inferior_t *inferior,
                                           uint64_t thread_id) {
  // @alwin: This should really be implemented with a hashmap
...
DebuggerError gdb_register_inferior(uint64_t inferior_id, seL4_CPtr vspace) {
  ...
  for (; curr_inferior_idx < end_idx; curr_inferior_idx++) {
    ...
    memset(inferior->software_breakpoints, 0,
           MAX_SW_BREAKS * sizeof(sw_break_t));
    memset(inferior->hardware_breakpoints, 0,
           seL4_NumExclusiveBreakpoints * sizeof(hw_break_t));
    memset(inferior->hardware_watchpoints, 0,
           seL4_NumExclusiveWatchpoints * sizeof(hw_watch_t));
...
DebuggerError gdb_register_thread(uint64_t inferior_id, uint64_t thread_id,
                                  seL4_CPtr tcb, char *output) {
  ...
  for (; inferior->curr_thread_idx < end_idx; inferior->curr_thread_idx++) {
    ...
    /* Set all the hardware breakpoints and watchpoints that have already been
       set for this inferior */
    ...
    if (!target_thread) {
      ...
      char *ptr = write_thread_id(thread, output + strnlen(output, BUFSIZE),
                                  BUFSIZE - strnlen(output, BUFSIZE));
      strlcpy(ptr, ";thread:", BUFSIZE);
      ptr = write_thread_id(target_thread, output + strnlen(output, BUFSIZE),
                            BUFSIZE - strnlen(output, BUFSIZE));
      strlcpy(ptr, ";", BUFSIZE);
    }
...
static void handle_set_inferior(char *ptr, char *output) {
  ...
  if (proc_id != PROC_ID_ANY) {
    ...
    // For now, we just leave the current thread as the target thread, which
    // seems fairly valid
  }
...
void handle_sig_interrupt(char *output) { strlcpy(output, "S02", BUFSIZE); }

bool handled[MAX_PDS][MAX_THREADS] = {0};

static void apply_vcont_to_all_threads(bool stepping) {
  for (int i = 0; i < MAX_PDS; i++) {
    if (!inferiors[i].enabled) {
      continue;
    }
    ...
    for (int j = 0; j < MAX_THREADS; j++) {
      ...
      if (stepping) {
        enable_single_step(&inferior->threads[j]);
      } else {
        disable_single_step(&inferior->threads[j]);
      }

      handled[GDB_INFERIOR_ID_TO_IDX(inferior->gdb_id)][j] = true;
      inferior->threads[j].wakeup = true;
    }
  }
}
...
void handle_vcont(char *input, char *output) {
  ...
  while (*input != 0) {
    ...
    if (*input == 's') {
      ...
      stepping = false;
      // @alwin: I think this is a bit dodgy. not entirely convinced that this
      // will work when there are both step and continue things in the same
      // package
      for (int i = 0; i < MAX_PDS; i++) {
        if (!inferiors[i].enabled)
          continue;
    ...
    if (*input != ':') {
      apply_vcont_to_all_threads(stepping);
      continue;
    }
    ...
    while (*input == ':') {
      ...
      /* If thread_id == -1, we want to apply the action to all the threads in
       * the inferior  */
      int i = 0;
      int n = MAX_THREADS;
      if (thread_id != THREAD_ID_ALL) {
...
static void handle_detach(char *ptr, char *output) {
  ...
  for (int i = 0; i < MAX_PDS; i++) {
    ...
    for (int i = 0; i < MAX_SW_BREAKS; i++) {
      unset_software_breakpoint(inferior,
                                inferior->software_breakpoints[i].addr);
    }
    for (int i = 0; i < seL4_NumExclusiveBreakpoints; i++) {
      unset_hardware_breakpoint(inferior,
                                inferior->hardware_breakpoints[i].addr);
    }
    for (int i = 0; i < seL4_NumExclusiveWatchpoints; i++) {
      unset_hardware_watchpoint(inferior,
                                inferior->hardware_watchpoints[i].addr,
...
bool gdb_handle_packet(char *input, char *output, bool *detached) {
  ...
  if (*input == 'g') {
    ...
    *detached = true;
    ...
    /* @alwin: This should probably report reasons other than swbreak, though
     * I've only seen this packet be used when first connecting to the system,
     * in which case only swbreak makes any sense.
     */
...
static void handle_ss_hwbreak_swbreak_exception(gdb_thread_t *thread,
                                                seL4_Word reason,
                                                char *output) {
  ...
  /* As we include a thread-id, GDB expects the target inferior to be the thread
   * that we set */
  target_thread = thread;
}

static void handle_watchpoint_exception(gdb_thread_t *thread, seL4_Word bp_num,
                                        seL4_Word trigger_address,
                                        char *output) {

  strlcpy(output, "T05thread:", BUFSIZE);
  char *ptr = write_thread_id(thread, output + strnlen(output, BUFSIZE),
                              BUFSIZE - strnlen(output, BUFSIZE));
  switch (thread->inferior->hardware_watchpoints[bp_num - seL4_FirstWatchpoint]
              .type) {
  ...
  ptr = mem2hex((char *)&vaddr_be, output + strnlen(output, BUFSIZE),
                sizeof(seL4_Word));
  strlcpy(ptr, ";", BUFSIZE);
...
static bool handle_debug_exception(gdb_thread_t *thread, seL4_Word *reply_mr,
                                   char *output) {
#ifndef MICROKIT
  ...
  seL4_Word trigger_address =
      microkit_mr_get(seL4_DebugException_TriggerAddress);
  ...
  // @alwin: i think you always want to resume for a debug exception. Think
  // about this more.
  return true;
}

static bool handle_fault(gdb_thread_t *thread, seL4_Word exception_reason,
                         char *output) {
  // @alwin: I'm pretty sure there is no fault here that should reawaken the
  // thread. Think about this more.
  // @alwin: Currentlywe just doing SIGABRT for every kind of fault that
  // happens, this probably could be better?
...
void resume_system() {
  for (int i = 0; i < MAX_PDS; i++) {
    ...
    for (int j = 0; j < MAX_THREADS; j++) {
      gdb_thread_t *thread = &inferior->threads[j];
      if (!thread->enabled || !thread->wakeup)
        continue;
...
DebuggerError gdb_handle_fault(uint64_t inferior_id, uint64_t thread_id,
                               seL4_Word exception_reason, seL4_Word *reply_mr,
                               char *output, bool *have_reply) {
  /* Make sure the inferior exists */
  gdb_inferior_t *inferior = lookup_inferior_from_id(inferior_id);
  if (!inferior) {
    return DebuggerError_InvalidArguments;
...
DebuggerError gdb_thread_exit(uint64_t inferior_id, uint64_t thread_id,
                              char *output) {
  /* Make sure the inferior exists */
  gdb_inferior_t *inferior = lookup_inferior_from_id(inferior_id);
  if (!inferior) {
