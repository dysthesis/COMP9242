...
/*
- <sel4/sel4_arch/syscalls.h> contains inline functions that use complex inline
assembly
- as of now writing this, Zig cannot translate them and will mark them as
`extern` stubs
- fix: do not inline these functions and ask Zig to inline them with LTO
- compatability: use the new syscall instruction `svc`
*/
#define static
#define inline __attribute__((always_inline))

/*
 * To simplify the definition of the various seL4 syscalls/syscall-wrappers we
 * define some helper assembly functions. These functions are designed to cover
 * the different cases of sending/receiving data in registers to/from the
 * kernel. The most 'complex' version is arm_sys_send_recv, and all other
 * functions are limited versions that allow for registers to not be
 * unnecessarily clobbered
 *
 * arm_sys_send: Fills all registers into the kernel, expects nothing to be sent
 * back by the kernel. Used for direction one way sends that contain data (e.g.
 * seL4_Send, seL4_NBSend)
 *
 * arm_sys_send_null: Only fills metadata registers into the kernel (skips
 * message registers). Expects nothing to be sent back by the kernel. Used by
 * directional one way sends that do not contain data (e.g. seL4_Notify)
 *
 * arm_sys_reply: Similar to arm_sys_send except it does not take a word for the
 *      destination register. Used for undirected one way sends that contain
 * data (e.g. seL4_Reply)
 *
 * arm_sys_recv: Sends one register (destination) to the kernel and expects all
 *      registers to be returned by the kernel. Used for directed receives that
 * return data (e.g. seL4_Recv)
 *
 * arm_sys_send_recv: Fills all registers into the kernel and expects all of
 * them to be filled on return by the kernel. Used for directed send+receives
 ...
 *      be returned from the kernel. Used to trigger implicit kernel actions
 * without any data (e.g. seL4_Yield)
 */

static inline void arm_sys_send(seL4_Word sys, seL4_Word dest,
                                seL4_Word info_arg, seL4_Word mr0,
...
static inline void arm_sys_reply(seL4_Word sys, seL4_Word info_arg,
                                 seL4_Word mr0, seL4_Word mr1, seL4_Word mr2,
                                 seL4_Word mr3) {
  register seL4_Word info asm("r1") = info_arg;
...
static inline void arm_sys_send_null(seL4_Word sys, seL4_Word src,
                                     seL4_Word info_arg) {
  ...
  /* Perform the system call. */
  register seL4_Word scno asm("r7") = sys;
  asm volatile("svc #0" : "+r"(destptr), "+r"(info) : "r"(scno));
}

static inline void arm_sys_recv(seL4_Word sys, seL4_Word src,
                                seL4_Word *out_badge, seL4_Word *out_info,
...
static inline void arm_sys_send_recv(seL4_Word sys, seL4_Word dest,
                                     seL4_Word *out_badge, seL4_Word info_arg,
                                     seL4_Word *out_info, seL4_Word *in_out_mr0,
                                     seL4_Word *in_out_mr1,
                                     seL4_Word *in_out_mr2,
                                     seL4_Word *in_out_mr3,
                                     LIBSEL4_UNUSED seL4_Word reply) {
  register seL4_Word destptr asm("r0") = dest;
  register seL4_Word info asm("r1") = info_arg;
...
#ifdef CONFIG_KERNEL_MCS
static inline void arm_sys_nbsend_recv(seL4_Word sys, seL4_Word dest,
                                       seL4_Word src, seL4_Word *out_badge,
                                       seL4_Word info_arg, seL4_Word *out_info,
                                       seL4_Word *in_out_mr0,
                                       seL4_Word *in_out_mr1,
                                       seL4_Word *in_out_mr2,
